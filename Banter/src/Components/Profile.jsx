...
function Profile(props) {
    ...
    return (
        <div className='bg-gray-100 w-[30vw]'> 
            <div className="bg-blue-500 text-white py-4 text-lg px-4 flex items-center gap-6">
            ...
            <div className="flex flex-col items-center justify-center gap-8 mt-8">
                <label className={` group relative cursor-pointer rounded-full overflow-hidden ${isUploading ? "pointer-events-none" : ""}`}>
                    ...
                    {isUploading ? (
                        <div className="absolute inset-0 flex items-center justify-center bg-black/10 z-10">
                            <Loader2Icon className="w-6 h-6 text-blue-700 animate-spin z-10" />
                        </div>
                    ) : (
                ...
                <div className="flex flex-col bg-white w-full py-4 px-8">
                    <label className="text-sm text-gray-700 mb-2">Your status</label>
                    <div className="flex items-center w-full">
                        <input
                            value={status}
                            className="w-full bg-transparent"
                ...
                <button onClick={handleLogout}
                    className='text-white px-4 py-3 rounded bg-blue-500 hover:bg-blue-700'
                >Logout</button>
            </div>
        </div>
    )
